...
目前 `deploy/argocd/application.yaml` 已經指向 `deploy/k8s`，但沒有任何流程負責「測試通過 → 建置 image → 推送 → 更新部署 manifest」這一段，等於 ArgoCD 有 GitOps 的目的地，卻沒有自動化的來源。這個 change 補上 CI/CD，讓 push 到 main 之後能自動跑完測試、建置、部署，不用每次手動操作。
...
- 新增 GitHub Actions workflow（push 到 main 觸發，排除只改 `deploy/k8s/**` 的 commit，避免自我觸發的無窮迴圈）：
  - 建置 `CoNotes.Api` 的 container image，以 git SHA 當 tag，推到 GitHub Container Registry（ghcr.io）
  - 更新 `deploy/k8s` 底下 API Deployment 的 image tag，commit 回 main（ArgoCD 的 `selfHeal` 會自動同步）
...
- `deploy/k8s` 底下 API 的 Deployment manifest 會被 CI 自動改動（image tag），人工修改這個檔案時要留意可能被下一次 CI 覆蓋。
