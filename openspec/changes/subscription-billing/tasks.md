...
> 6.0/6.3 都完成了。你提供了一組 PayPal sandbox buyer 帳號後，用 Playwright 跑了一次完整真實流程：真的登入 PayPal 核准頁、真的核准付款、真的導回、真的 capture、真的 webhook 送達與簽章驗證、真的產生 code、真的兌換、真的解鎖分享連結功能——中途也真的抓到一個之前完全沒發現的 bug（見 6.3 說明）。
>
> **過程中發現的真的 bug**：`PayPalClient.CaptureOrderAsync` 呼叫 Capture API 時完全沒帶 body/`Content-Type` header，之前只用「還沒核准的訂單」測過這條路徑（PayPal 對這種請求回 422 `ORDER_NOT_APPROVED`，剛好掩蓋了問題），第一次用真的已核准訂單測試時，PayPal 回 `415 UNSUPPORTED_MEDIA_TYPE`：「這個 endpoint 就算沒有欄位要帶，還是要求明確宣告 `Content-Type: application/json`」。這個 bug 在此之前完全沒被抓到，因為所有既有測試都只測到「未核准」這個分支，從沒有真的用一個已核准、可以成功 capture 的訂單測過。修法：`request.Content = JsonContent.Create(new { })`，帶一個空的 JSON body 觸發正確的 Content-Type。同時把 `PayPalClient` 加上 `ILogger`，capture 失敗時記下 PayPal 實際回傳的狀態碼與內容——原本這裡是完全沒有任何記錄的，這個 bug 能被抓到純粹是因為手動加了診斷 log 才看到 415 錯誤，之後應該保留這個 log 陳述以利未來排查。

- [x] 6.0 取得一組 PayPal sandbox buyer 帳號（email/密碼），供 Playwright 自動化核准流程使用
- [x] 6.1 逐一驗證 `specs/subscription-billing/spec.md` 的五個 Requirement 全數通過（「透過 PayPal 付款取得 code」：`PayPalClientTests`(真的打 sandbox API 建立訂單/capture/驗證 webhook 簽章) + `IssueLicenseCodeCommandHandlerTests`(含 idempotency) + `BillingEndpointTests`(未核准就 confirm 正確拒絕、webhook endpoint 正確驗證簽章) + **真的用 buyer 帳號跑完整購買流程**(見 6.3)；「兌換有效 code」：`RedeemLicenseCodeCommandHandlerTests` + `BillingEndpointTests` + Playwright 實測(含真的用 PayPal 發出來的 code 兌換)；「已兌換的 code 不能再兌換」：`RedeemLicenseCodeCommandHandlerTests`(`GivenAlreadyRedeemedCode...`) + `BillingEndpointTests`(`GivenAlreadyRedeemedCode...`)；「已兌換等級不隨 PayPal 後續狀態變動」：`RedeemLicenseCodeCommandHandlerTests` 三個測試合起來證明的不變條件(見 2.4)；「管理者手動撤銷」：`RevokeAppUserPlanTierCommandHandlerTests` + `BillingEndpointTests` + `LicenseCodeRedemptionTests`(真的對 Postgres 驗證撤銷後重新讀回是 Free)。完整 `CoNotes.slnx` 135/135 通過)
- [x] 6.2 逐一驗證 `specs/collab-editing/spec.md`、`specs/ai-chat/spec.md` 這次修改的部分全數通過（分享連結需要 Pro 以上：`GenerateShareLinkCommandHandlerTests` 的 Free/Pro/ProMax 三種情況 + Playwright 實測(Free 被拒、兌換真的 PayPal 付款產生的 Pro code 後成功)；聊天室需要 ProMax：`SendChatMessageCommandHandlerTests` 的 Free/Pro/ProMax 三種情況 + Playwright 實測(Pro 被拒並顯示提示)；共編者/聊天室參與者不需要自己也符合等級：沿用既有的 owner-only 檢查、共編者本來就不會被查 `PlanTier`，`NoteCollaborationTests`/`ChatMessageTests` 的既有共編者情境未受影響；軟性降級(撤銷後既有共編者/聊天記錄不受影響)：`GivenOwnerPlanTierRevoked_WhenQueryingExistingCollaboratorsOrChatHistory_ThenExistingDataUnaffected` 整合測試)
- [x] 6.3 完整跑一次流程：PayPal sandbox 付款購買 Pro → 核准並導回確認 → webhook 送達並驗證簽章 → 產生 code → 輪詢拿到 code → 兌換 → 產生分享連結成功。用 Playwright 全程自動化跑過：建立臨時 Keycloak 使用者 → 建立真的 PayPal 訂單 → 導向真的核准頁、用你提供的 sandbox buyer 帳號真的登入並按下付款按鈕 → 真的導回確認頁 → 前端輪詢真的拿到 PayPal webhook 觸發產生的 code(不是測試專用 endpoint 發的) → 兌換成功、畫面顯示「你的訂閱等級已更新為 Pro」→ 建立筆記、產生分享連結成功。ProMax 購買聊天室的部分沒有另外重複整條真實購買流程——PlanTier 判斷邏輯與 Pro 那條路徑完全共用同一組程式碼(`IssueLicenseCodeCommand`/`RedeemLicenseCodeCommand` 不分等級)，Pro 已經證明整條鏈路真的走得通，ProMax 只是 `custom_id` 的值不同，這部分已經由 `SendChatMessageCommandHandlerTests`/`BillingEndpointTests` 的自動化測試涵蓋
