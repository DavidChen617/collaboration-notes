...
**Non-Goals:**
- 追蹤 PayPal 訂閱的完整生命週期（續訂成功、扣款失敗、取消等事件）——這裡只處理「首次訂閱啟用成功」這一個事件，其餘決定不處理（見 Decisions）。
- 對已經加入協作/聊天室的共編者做等級檢查——這個 change 只檢查筆記擁有者的等級。
- 使用者自己降級（自助取消訂閱、自助退回 Free）——目前只有系統管理者能手動撤銷。
...
## Sequence：訂閱付款、兌換、與跨 Context 的等級檢查
...
sequenceDiagram
    ...
    PayPal->>Api: webhook：訂閱首次啟用成功
    Api->>IssueCmd: IssueLicenseCodeCommand(PlanTier, PayPalSubscriptionId)
    ...
    AppUser-->>Evt: 完成（往後 PayPal 狀態變化不再觸發任何動作）
...
**2. License code 是一次性解鎖，兌換生效後不隨 PayPal 之後的扣款/取消狀態自動變動；因此系統只監聽 PayPal「訂閱首次啟用成功」這一個 webhook 事件，其餘續訂/取消/扣款失敗事件都不處理。**
這是跟你確認過的決定。不追蹤完整訂閱生命週期，大幅簡化資料模型——不需要一張獨立的 `Subscription` table 去記錄扣款狀態，PayPal 的訂閱 id 只要附記在觸發它產生的那筆 `LicenseCode` 上，當作稽核用途即可。

**3. `LicenseCode` 的兌換不限制兌換者身份——任何登入的使用者輸入一組尚未使用的有效 code，都能把該 code 對應的等級套用到自己的帳號上。**
Code 的性質類似禮品卡：誰有這組碼、誰就能兌換。付款的人跟兌換的人不一定要是同一人（例如可以把碼送給別人），這裡不特別限制。

**4. 撤銷等級是系統管理者的手動操作，把指定使用者的 `PlanTier` 設回 `Free`。**
因為決定 2 讓等級不會自動跟著 PayPal 狀態變動，勢必要有一個獨立的收回手段，這裡選最簡單的方式：手動操作，不做自動化。

**5. `collab-editing` 與 `ai-chat` 的等級檢查，只檢查「筆記擁有者」的 `PlanTier`，不檢查共編者/聊天室參與者自己的等級。**
...
**6. 擁有者的等級被撤銷或降級後，只會擋住「之後」的受限動作（例如不能再產生新的分享連結、`@AI` 不會再回應），不會回溯移除既有的共編者名單或刪除既有的聊天紀錄。**
降級是「軟性」的——不做破壞性清理，避免使用者資料無預警消失。曾考慮「降級就把共編者全部踢掉、清空聊天記錄」——否決，這種做法對使用者不友善，也不是這個 change 需要處理的急迫問題。

## Risks / Trade-offs

- **[Risk]** 使用者兌換 code 拿到 Pro/ProMax 之後，就算之後 PayPal 那邊取消訂閱或扣款失敗，等級也不會自動收回——變成「付一次月費、永久解鎖」的實質效果。→ **Mitigation**：這是你已經確認接受的取捨；管理者手動撤銷是唯一的收回手段；玩具/學習專案定位下可以接受，正式商業化前需要重新設計。
- **[Risk]** License code 兌換不限制身份，一組 code 外流就可能被非付款人搶先兌換。→ **Mitigation**：code 是一次性的，最多被搶兌換一次；這跟禮品卡的性質一致，是可接受的取捨。
- **[Risk]** 只監聽「首次啟用成功」這一個 PayPal webhook 事件，代表完全不知道使用者後續是否還在正常付款。→ **Mitigation**：這是刻意接受的範圍縮減；之後如果需要，可以再加其他事件類型，不影響現在的 `LicenseCode` 資料模型。
