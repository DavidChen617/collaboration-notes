...
`collab-editing`（Pro）跟 `ai-chat`（ProMax）目前對所有使用者開放，沒有訂閱等級限制——這個 change 補上付款與等級解鎖機制，把這兩個功能真正變成付費分級的樣子。

（原本規劃走 PayPal Subscriptions API 的定期訂閱，後來跟你確認過改成單次付款：PayPal 這裡只當一次性收費用，不用它的訂閱/定期扣款機制。）

## What Changes

- 使用者可以透過 PayPal 一次性付款（Orders API，sandbox）購買 Pro 或 ProMax 等級。
- PayPal 付款完成後，系統產生一組對應等級的 license code。
- 使用者輸入尚未使用過的 license code 後，系統把該使用者的訂閱等級更新為 code 對應的等級——這是**一次性解鎖**：之後這筆 PayPal 付款被退款或申訴爭議（chargeback），都不會自動改變已經兌換生效的等級。
- 系統管理者可以手動把使用者的訂閱等級撤銷回 Free（因為等級不會隨 PayPal 付款後續狀態自動變動，需要一個獨立的撤銷手段）。
...
- 新增 API：建立 PayPal 訂單（Orders API）checkout、PayPal webhook 接收端點、兌換 license code、管理者撤銷等級。
